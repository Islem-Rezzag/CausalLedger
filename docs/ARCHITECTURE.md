...
- Canonical events: future MoneyEvent schemas and transformations. The conceptual boundary is documented in `docs/MONEYEVENT_CONTRACT.md`, and `packages/events` now contains the M03.02 TypeScript type boundary only; no runtime MoneyEvent behavior exists yet.
