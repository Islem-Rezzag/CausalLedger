...
| Ledger core | Partial - account schema only | M04.01 in progress: strict Account metadata types/validation and supplied-catalog duplicate detection in `packages/ledger`; no durable uniqueness, authorization, transactions, entries, posting, balances, reversals or storage. See `docs/specs/account-schema.md` and the active M04 plan. |
