...
| Local infrastructure baseline | Builder complete, awaiting QA | M02.06 adds local-only Docker Compose/Postgres, empty env example placeholders, `node-pg-migrate` commands, an empty migration boundary, and `/infra/ready` infrastructure readiness only; no product schema, storage behavior, Redis, queues, schedulers, or production deployment |
