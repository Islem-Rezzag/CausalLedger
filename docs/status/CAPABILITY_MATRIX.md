...
| Monorepo and local development | In progress | Active plan exists at `plans/active/CLP-0003-m02-monorepo-and-local-development-environment.md`; M02.01 and M02.02 are completed and merged, M02.03 is QA passed awaiting merge, and M02.04 through M02.20 remain Not started |
...
| Web app foundation | QA passed, awaiting merge | `apps/web` has a minimal non-domain React/Vite scaffold on branch `m02-03-create-apps-web`; PR #40 is safe for human merge after QA; no CausalLedger product UI, API calls, routing, auth/authz, database behavior, or domain behavior |
