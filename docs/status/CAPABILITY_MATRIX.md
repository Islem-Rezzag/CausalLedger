...
| Monorepo and local development | In progress | Active plan exists at `plans/active/CLP-0003-m02-monorepo-and-local-development-environment.md`; M02.01, M02.02, and M02.03 are completed and merged, M02.04 is builder complete awaiting QA, and M02.05 through M02.20 remain Not started |
...
| Worker app foundation | Builder complete, awaiting QA | `apps/worker` has a minimal non-domain TypeScript scaffold on branch `m02-04-create-apps-worker`; no jobs, queues, schedulers, provider connectors, database behavior, health checks, or domain behavior |
