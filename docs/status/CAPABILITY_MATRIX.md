...
| MoneyEvent engine | M03.05 QA passed, awaiting merge | M03.04 is completed and finalized at `4afa9e94bc3938e3138ce2045afc380582b24c71`. `packages/events` contains compile-time types plus deterministic source-neutral candidate validation and normalization. M03.05 PR #55 adds 21 controlled synthetic fixtures, seven early MoneyFlowBench seed cases, and strict test-only verification; no source-specific parser or mapper, ingestion, storage, database, API, ledger, invariant, incident, graph, replay, repair, agent, connector, UI, benchmark scoring, or financial-truth behavior. M03.06 remains not started. |
