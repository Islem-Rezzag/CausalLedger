...
Thread name: `M01.07 QA - Define Evidence Receipt Model`
...
Goal: QA the M01.07 evidence receipt model as documentation-only domain vocabulary and tracking updates.

Current boundary: M01 is active. M01.06 is `Completed and merged`. M01.07 is `Builder complete, awaiting QA`. M01.08 through M01.13 are `Not started`. M02 through M21 are `Not started`. Product implementation has not started.

The M01.07 QA thread may inspect the evidence receipt model doc, domain links, status files, milestone tracking, active plan notes, and validation coverage for the evidence receipt model only. It must keep M01.08 through M01.13 and M02 through M21 `Not started`.

Do not start M02, product implementation, MoneyEvent runtime logic, ledger runtime logic, settlement runtime logic, reconciliation runtime logic, incident runtime logic, invariants, graph, replay, agent runtime, repair planning runtime logic, repair execution, UI, external connectors, APIs, databases, storage layers, file parsers, GitHub Actions, CI workflows, deployment, auth/authz implementation, structured logging implementation, or secrets work from the M01.07 QA thread.
