...
Product domain implementation has not started. `apps/api` now contains a minimal non-domain TypeScript/Fastify scaffold and bootstrap test, while other future app/package/product directories remain placeholders. `.github/workflows/` does not exist.

## Next action

Run the exact next recommended thread: `Merge M02.02 PR - Create apps/api`.

## Implementation warning

Do not start product domain implementation. The next thread must merge M02.02 PR #39 only after human review and must not create product behavior, MoneyEvent runtime code, ledger runtime logic, settlement runtime logic, reconciliation runtime logic, incident runtime logic, invariants, graph logic, replay logic, agent runtime, repair planning runtime logic, repair execution, human-review runtime logic, AML/KYC behavior, sanctions screening, fraud scoring, credit decisions, legal or tax advice, investment advice, UI features, CausalLedger domain API routes, databases, storage layers, file parsers, external connectors, GitHub Actions, CI workflows, runtime authentication or authorization, RBAC, encryption, secret management, audit logs, structured logging implementation, monitoring, runtime security controls, health checks reserved for M02.18, or product behavior. Do not start M02.03 until the M02.02 PR merges into `main` and post-merge tracking is finalized.
