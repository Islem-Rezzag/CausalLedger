# Tech Debt

No product tech debt exists yet because product implementation has not started.

## Placeholders to resolve later

- `docs/DOMAIN_MODEL.md` is now an M01 domain index; remaining M01.09 through M01.13 domain areas stay intentionally undefined until their scoped submilestones.
- Choose monorepo tooling.
- Choose test framework conventions.
- Define schema language and package boundaries.
- Define local development services.
