...
`@causalledger/repair` is the future repair proposal package boundary.
...
No CausalLedger product or domain behavior is implemented here. There are no financial schemas, ledger entries, balances, invariants, incident workflows, graph traversal, replay algorithms, repair logic, repair approval, evidence storage, benchmark behavior, external connectors, agents, database behavior, or money mutation.
