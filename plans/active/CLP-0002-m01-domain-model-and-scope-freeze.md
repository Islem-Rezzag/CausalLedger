...
- [x] 2026-05-21: Finalized M01.10 as `Completed and merged` after QA recovery PR #29 merged at git commit `a878d55` (`test: QA recovery M01.10 domain model summary (#29)`) before M01.11 builder work began.
- [x] 2026-05-21: Started M01.11 builder on branch `m01-11-write-reliability`; branch guard passed, the starting worktree was clean, and M01.11 was marked `Builder in progress`.
- [x] 2026-05-21: Rewrote `docs/RELIABILITY.md` as the canonical CausalLedger reliability model for deterministic financial checks, evidence provenance, replay, repair validation, human review, audit trails, bounded AI assistance, model routing, cost reliability, observability, ablations, failure modes, metrics, future dependencies, and guardrails.
- [x] 2026-05-21: Updated README, docs index, domain model, active plan, roadmap, milestone doc, submilestone registry, current state, next recommended thread, weekly log, capability matrix, tech debt, risk register, validation script, and bootstrap tests for M01.11.
- [x] 2026-05-21: Initial M01.11 `python -m pytest tests/test_control_plane_bootstrap.py` failed because the new reliability-model test treated the required negative statement `LLM output is evidence` as a forbidden positive claim; the scoped test assertion was narrowed and validation was rerun successfully.
- [x] 2026-05-21: M01.11 builder validation passed with `python scripts/validate-control-plane.py`, `python -m pytest tests/test_control_plane_bootstrap.py` with 28 tests, and `git diff --check`; `make bootstrap-check` was skipped because `make` is unavailable in the current Windows shell.
- [x] 2026-05-21: Updated M01.11 tracking to `Builder complete, awaiting QA`; M01.12 and M01.13 remain `Not started`; M02 through M21 remain `Not started`; product implementation has not started.
- [x] 2026-05-21: Completed M01.11 QA on branch `m01-11-write-reliability`; branch guard passed, starting worktree was clean, latest commit was `518d8a8`, remote `origin` points to GitHub, and tag `v0.1.0` exists.
...
- [x] 2026-05-23: Completed M01.12 QA on branch `m01-12-write-threat-model`; branch guard passed, the starting worktree was clean, latest commit was `18a9aa7`, remote `origin` points to GitHub, and tag `v0.1.0` exists.
- [x] 2026-05-23: QA verified `docs/THREAT_MODEL.md`, tracking/status files, validation coverage, and forbidden-scope boundaries; no threat-model content defects and no product implementation were found.
- [x] 2026-05-23: QA fixes were limited to tracking/status and control-plane validation expectation updates, including correcting the stale active-plan outcome summary.
- [x] 2026-05-23: M01.12 QA validation passed with `python scripts/validate-control-plane.py`, `python -m pytest tests/test_control_plane_bootstrap.py` with 29 tests, and `git diff --check`; `make bootstrap-check` was skipped because `make` is unavailable in the current Windows shell.
- [x] 2026-05-23: Updated M01.12 tracking to `QA passed, awaiting merge`; M01.13 remains `Not started`; M02 through M21 remain `Not started`; product implementation has not started.
- [x] 2026-05-23: Finalized M01.12 as `Completed and merged` after PR #31 merged; duplicate PR merges #32 and #33 were also created from the same M01.12 branch, which is recorded as a process deviation.
- [x] 2026-05-23: No revert is being done for duplicate M01.12 PR merges #32 and #33; corrective action is to restore tracking consistency and continue with stricter PR discipline: one PR per branch and branch deletion after merge for future submilestones.
...
- [x] 2026-05-23: Updated domain model, reliability, threat model, entry docs, active plan, roadmap, milestone doc, registry, status docs, capability matrix, tech debt, validation script, and bootstrap tests for M01.13.
- [x] 2026-05-23: M01.13 builder validation passed with `python scripts/validate-control-plane.py`, `python -m pytest tests/test_control_plane_bootstrap.py`, and `git diff --check`; `make bootstrap-check` was skipped because `make` is unavailable in the current Windows shell.
- [x] 2026-05-23: Updated M01.13 tracking to `Builder complete, awaiting QA`; M01 is not complete, M01 closeout still requires M01.13 QA PASS and PR merge, M02 through M21 remain `Not started`, and product implementation has not started.
- [x] 2026-05-25: Completed M01.13 QA on branch `m01-13-qa-domain-consistency`; branch guard passed, the starting worktree was clean, latest commit was `254bdcd`, remote `origin` points to GitHub, and tag `v0.1.0` exists.
- [x] 2026-05-25: QA verified the domain consistency report, DOMAIN_MODEL, RELIABILITY, THREAT_MODEL, M01 domain docs, eval docs, spec placeholders, roadmap, registry, active plan, status docs, validation coverage, and forbidden-scope boundaries.
- [x] 2026-05-25: QA fixed the scoped checked-file list defect in `docs/status/M01_DOMAIN_CONSISTENCY.md` and updated tracking/status files to `QA passed, awaiting merge`.
- [x] 2026-05-25: M01.13 QA validation passed with `python scripts/validate-control-plane.py`, `python -m pytest tests/test_control_plane_bootstrap.py` with 30 tests, and `git diff --check`; `make bootstrap-check` was skipped because `make` is unavailable in the current Windows shell.
- [x] 2026-05-25: M01 remains active, M01.13 is not completed or merged, PR #35 is open, M01 closeout still requires M01.13 PR merge, M02 through M21 remain `Not started`, and product implementation has not started.
...
- 2026-05-23: PR #31 merged M01.12; duplicate PR merges #32 and #33 from the same M01.12 branch were also created. This is a process deviation; do not revert in this finalization thread. Restore tracking consistency and use one PR per branch plus branch deletion after merge for future submilestones.
...
- M01.11 runtime reliability mechanisms or product behavior.
- M01.12 implementation.
- M02 implementation.
...
Completed M01.11 builder work:

1. Finalize M01.10 as `Completed and merged` from git history at `a878d55` (`test: QA recovery M01.10 domain model summary (#29)`).
2. Mark M01.11 as `Builder in progress` on branch `m01-11-write-reliability`.
3. Rewrite `docs/RELIABILITY.md` as the canonical CausalLedger reliability model.
4. Update concise entry-doc references and tracking files.
5. Add documentation-only validation checks for the expanded `docs/RELIABILITY.md`.
6. Run validation and record results.
7. Mark M01.11 `Builder complete, awaiting QA`.
...
- M01.10 is `Completed and merged` after QA recovery PR #29 merged at git commit `a878d55`.
...
- Next recommended thread after M01.11 QA is `Merge M01.11 PR - Write RELIABILITY.md`.
...
- 2026-05-21 M01.10 post-merge finalization:
  - QA recovery PR #29 was confirmed merged at git commit `a878d55` (`test: QA recovery M01.10 domain model summary (#29)`) before M01.11 builder work began.
  - Tracking updates in this M01.11 builder slice record M01.10 as `Completed and merged`.
- 2026-05-21 M01.11 builder validation results:
...
- 2026-05-21 M01.11 QA validation results:
  - QA reviewed `docs/RELIABILITY.md`, tracking/status files, validation coverage, and forbidden-scope boundaries.
  - No reliability model content defects and no product implementation were found.
  - `python scripts/validate-control-plane.py` passed.
  - `python -m pytest tests/test_control_plane_bootstrap.py` passed with 28 tests.
  - `git diff --check` passed.
  - `make bootstrap-check` was skipped because `make` is unavailable in the current Windows shell.
  - M01.11 was marked `QA passed, awaiting merge`; it was not `Completed and merged` until PR #30 merged.
- 2026-05-21 M01.11 post-merge finalization:
  - PR #30 was confirmed merged at git commit `a424924` (`docs: write M01.11 reliability model (#30)`) before M01.12 builder work began.
  - Tracking updates in the M01.12 builder slice record M01.11 as `Completed and merged`.
- 2026-05-21 M01.12 builder validation results:
  ...
  - `git diff --check` passed.
  - `make bootstrap-check` was skipped because `make` is unavailable in the current Windows shell.
  - M01.12 was marked `Builder complete, awaiting QA`; M01.13 remained `Not started`; product implementation did not start.
- 2026-05-23 M01.12 QA validation results:
  - QA reviewed `docs/THREAT_MODEL.md`, tracking/status files, validation coverage, and forbidden-scope boundaries.
  - No threat-model content defects and no product implementation were found.
  ...
  - M01.13 remains `Not started`; M02 through M21 remain `Not started`; product implementation has not started.
- 2026-05-23 M01.12 post-merge finalization:
  - PR #31 merged M01.12.
  - Duplicate PR merges #32 and #33 were also created from the same M01.12 branch; this is recorded as a process deviation.
  - No revert is being done in this finalization thread.
  - Corrective action is to restore tracking consistency, continue with stricter PR discipline, create only one PR per branch for future submilestones, and delete the branch after merge.
...
No product runtime interface is introduced by this M01 planning, M01.01 documentation work, M01.02 documentation work, M01.03 documentation work, M01.04 documentation work, M01.05 documentation and evaluation-planning work, M01.05 QA recovery work, M01.05 post-merge finalization work, M01.06 repair vocabulary work, M01.07 evidence receipt model work, M01.07 post-merge finalization work, M01.08 human review states work, M01.08 post-merge finalization, M01.09 out-of-scope domain builder work, M01.09 QA work, M01.09 post-merge finalization, M01.10 DOMAIN_MODEL.md builder work, M01.10 QA recovery work, M01.10 post-merge finalization, M01.11 RELIABILITY.md builder work, M01.11 QA work, M01.11 post-merge finalization, M01.12 THREAT_MODEL.md builder work, or M01.12 QA work.
